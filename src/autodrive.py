...
class NetworkNvidia(nn.Module): #get the trained model and make the autodrive. 
...
if __name__ == '__main__':
    ...
    model = NetworkNvidia() #declare the CNNnetwork.
    ...
    # cap = cv2.VideoCapture(1) # first camera.
    cap = cv2.VideoCapture("./juno_dorm1.mp4")
    ...
    while True:
        ...
        cv2.imwrite('frame.jpeg', frame_roi)
        ...
        print("prediction", steering_angle)
        steering_velocity = model(image_tensor).view(-1).data.numpy()[1]
        print("ang gi ho Ttivel prediction",steering_velocity)

            #steering_angle 값을 quantization을 해야함
        #steering_angle = steering_angle-0.253
        steering_angle = steering_angle * 20

        print("actuall angle : ", steering_angle)
